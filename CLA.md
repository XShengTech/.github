# XShengTech Contributor License Agreement (CLA)

_Applies to all repositories under the `XShengTech` GitHub organization._

By submitting a pull request or contributing code, documentation, or any other material (“Contribution”) to any repository under the **XShengTech** organization, you agree to the following terms:


## **1\. License Grant**

You hereby grant **XShengTech** an irrevocable, worldwide, perpetual, royalty-free license to:

- Use, modify, reproduce, distribute, and sublicense your Contribution
    
- Create derivative works based on your Contribution
    
- Re-license the Contribution under **multiple licenses**, including but not limited to:
    
    - AGPLv3
        
    - Commercial proprietary licenses
        
    - Any other licenses selected by XShengTech
        

This enables XShengTech to maintain both open-source and commercial versions of its software.


## **2\. Ownership & Rights**

You confirm that:

- You are the original author of the Contribution, **or**
    
- You have sufficient rights to grant this license.
    

You retain the copyright of your Contribution.


## **3\. No Obligation to Use the Contribution**

XShengTech is not required to incorporate or distribute your Contribution.


## **4\. No Warranty**

Your Contribution is provided “as is” without warranties.

---

# XShengTech 贡献者许可协议（CLA）

> 中文版本（仅供参考，以英文为准）

适用于 XShengTech 组织下的所有项目。

当你向本组织提交 Pull Request 或其他贡献（代码、文档等）时，你同意：


## **1\. 版权许可**

你授予 XShengTech：

- 不可撤销、全球范围、永久、免费的许可
    
- 使用、修改、分发你的贡献
    
- 创建衍生作品
    
- 以 **AGPL、商业闭源许可等多种许可证** 发布包含你贡献的版本



## **2\. 所有权声明**

你保证贡献内容：

- 为你的原创作品，或
    
- 你拥有提交该作品的合法权利
    

你仍然保留对贡献部分的版权。


## **3\. 无纳入义务**

XShengTech 不承担合并或发布贡献的义务。


## **4\. 免责声明**

贡献按“原样”提供，不保证适用性或无缺陷。


提交 Pull Request 即表示你已同意本协议的条款。